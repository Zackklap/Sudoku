 ...
 *
 * your name: Zack 'pogchamp' Klappenbach
 *
 */
...
public class Sudoku {
    ...
    private boolean[][][] gridHasVal;
    private boolean[][] rowHasVal;
    private boolean[][] colHasVal;
    private final int[] goodVal = new int[]{1,2,3,4,5,6,7,8,9};
    ...
    public void placeVal(int val, int row, int col) {
        ...
        this.gridHasVal[row][col][val]=true;
    ...
    public void removeVal(int val, int row, int col) {
        ...
        /*** UPDATE YOUR ADDITIONAL FIELDS HERE. ***/
        this.gridHasVal[row][col][val]=false;
    ...
    private boolean solveRB(int n) {
        boolean done=true;
        int count=0;
        for(int i =0; i<subgridHasVal.length; i++)
        {
            for (int j=0; j<subgridHasVal[i][j].length;i++){
            if(grid[i][j]==0){
                    done = false;
                    break;
                }
    ...
    public boolean isSafe(int row, int col, int n){
        if(!subgridHasVal[row/3][col/3][n] &&  !colHasVal[col][n] && !rowHasVal[row][n])
                return true;
        return false;
    }
